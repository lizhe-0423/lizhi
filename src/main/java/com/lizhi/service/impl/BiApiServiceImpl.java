...
public class BiApiServiceImpl extends ServiceImpl<BiApiMapper, BiApi>
    ...
    public BiApi getSignature(BiApiSignatureRequest biApiSignatureRequest) {
        ...
        String message = "请帮我生成一个个性签名 类型:"
                ...
                + style
                + "字数:" + words;
        return getBiApi(signatureId, type, style, message, "签名", user.getUserId(), accessKey, secretKey);
    }
    ...
    public BiApi getTranslation(BiApiTranslationRequest biApiTranslationRequest) {
        ...
        String message = "请帮我翻译以下内容(只需要翻译内容:后的内容 不需要翻译其余内容) 类型(决定翻译成什么语言):"
                ...
                + style + "内容:"
                + text;
        return getBiApi(translationId, type, style, message, "翻译", user.getUserId(), accessKey, secretKey);
    }
    ...
    public BiApi getCopy(BiCopyRequest biCopyRequest) {
        ...
        return getBiApi(copyId, type, style, message, "文案", user.getUserId(), accessKey, secretKey);
    }

    @Override
    public BiApi getBiApi(Long id, String type, String style, String message, String name, Long userId, String userAccessKey, String userSecretKey) {
        BiApi biApi = new BiApi();
        ...
        String doChat = aiManage.doChat(id, message, userAccessKey, userSecretKey);
